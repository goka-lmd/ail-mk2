...
from ail_mk2.base_il2 import BaseILAlgo2
from rlf.args import str2bool
from rlf.storage.base_storage import BaseStorage
from tqdm import tqdm


class GCPC(BaseILAlgo2):
    """
    ...
    def get_num_updates(self):
        if self.exp_generator is None:
            return len(self.expert_train_loader) * self.args.gcpc_num_epochs
        else:
            return self.args.exp_gen_num_trans * self.args.gcpc_num_epochs
    ...
    def _bc_step(self, decay_lr):
        ...
        if expert_batch is None:
            ...
            expert_batch = self._get_next_data()
        states, observations, padding_mask, true_actions = self._get_data(expert_batch)

        log_dict = {}

        pred_actions, _, _ = self.policy(states, observations, padding_mask, None, None)
    ...
    def _get_data(self, batch):
        past_states = batch["past_states"].to(self.args.device)
        states = batch["state"].to(self.args.device)
        padding_mask = batch["padding_mask"].to(self.args.device)
        if self.args.gcpc_state_norm:
            states = self._norm_state(states)

        if self.args.gcpc_noise is not None:
            add_noise = torch.randn(states.shape) * self.args.gcpc_noise
        ...
        return states, past_states, padding_mask, true_actions
    ...
    def _compute_val_loss(self):
        ...
        with torch.no_grad():
            ...
            for batch in self.val_train_loader:
                states, observations, padding_mask, true_actions = self._get_data(batch)
                pred_actions, _, _ = self.policy(states, observations, padding_mask, None, None)
                loss = autils.compute_ac_loss(
                    pred_actions,
                    true_actions.view(-1, self.action_dim),
    ...
    def get_add_args(self, parser):
        ...
        #########################################
        # New args
        parser.add_argument("--gcpc-num-epochs", type=int, default=1)
        parser.add_argument("--gcpc-state-norm", type=str2bool, default=False)
        parser.add_argument("--gcpc-noise", type=float, default=None)
