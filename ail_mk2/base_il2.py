...
from ail_mk2.transition_dataset_mk2 import TransitionDataset
    ...
    def pt_collate_fn(batch):
        ...
        for key in batch[0].keys():
            ...
            if key == "past_states":
                ...
                past_state_lengths = [v.shape[0] for v in values]

                max_len = padded.size(1)
                mask = torch.ones((len(values), max_len), dtype=torch.bool)
                for i, length in enumerate(past_state_lengths):
                    mask[i, :length] = False
                ...
                try:
                    ...
                    print(f"Warning: Failed to stack key '{key}': {e}")
                    batch_dict[key] = values  # fallback: keep as list
        return batch_dict
